...
type SHA256Content struct {
	x string
}

//CalculateHash hashes the values of a TestSHA256Content
func (t SHA256Content) CalculateHash() ([]byte, error) {
	h := sha256.New()
...
func (t SHA256Content) Equals(other Content) (bool, error) {
	return t.x == other.(SHA256Content).x, nil
}
...
func GenerateMerkleTree(txs []meta.Transaction) *MerkleTree {
	...
	newTree, _ := NewTree(contents)
	return newTree
}

//计算一个区块的merkle root
func CalBlockMerkleRoot(b meta.Block) (merkleRoot []byte) {
	//先取出所有的交易
	txs := b.TX
	contents := make([]Content, 0)
	for _, tx := range txs {
	...
	newTree, _ := NewTree(contents)
	return newTree.merkleRoot
}

//生成某一笔交易的merkle path等
func GetTranMerklePath(txs []meta.Transaction, sequence int) (hash []byte, merklePath [][]byte, index []int64) {
	...
	var c SHA256Content
	c.x = txStr
	merklePath, index, _ = newTree.GetMerklePath(c)
	return hash, merklePath, index
}

//验证某一笔交易是否在区块中
func VerifyTranExistence(hash []byte, merklePath [][]byte, index []int64, merkleRoot []byte) bool {
	var err error
	h := sha256.New()
	...
	if bytes.Compare(merkleRoot, hash) != 0 {
		...
		log.Info("Merkle Verification Success")
		return true
	}
}
