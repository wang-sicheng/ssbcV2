...
func ReadData(rw *bufio.ReadWriter) {
	for {
		...
		if str != "\n" {
			...
			if msg.Type == commoncon.BlockChainSynchronizeMsg {
				//区块同步消息
				handleBlockChainSynchronizeMsg(msg.Content)
			} else if msg.Type == commoncon.AbstractBlockHeaderSynchronizeMsg {
				//抽象区块头同步消息
				handleAbstractBlockHeaderSynchronizeMsg(msg.Content)
			} else if msg.Type == commoncon.VRFOrderMsg {
				//vrf指令处理
				handleVRFOrderMsg(msg.Content, rw)
			} else if msg.Type == commoncon.VRFMsg {
				//vrf结果消息处理
				handleVRFMsg(msg.Content, rw)
			} else if msg.Type == commoncon.TssMsg {
				//针对提案的门限签名的消息处理
				//节点收到提案，基于提案进行签名发给提案节点，逐一验证，达到阈值后签名成功
			} else if msg.Type == commoncon.AbstractHeaderProposalMsg {
				//收到提案节点的提案消息的处理
				handleAbstractHeaderProposalMsg(msg.Content, rw)
			} else if msg.Type == commoncon.HeaderProposalSignMsg {
				//提案节点收到针对提案的签名
...
func summitAbHeaders(headers []byte) {
	redis.SetIntoRedis(commoncon.AbstractHeadersFinalKey, string(headers))
...
func handleAbstractHeaderProposalMsg(content string, rw *bufio.ReadWriter) {
	...
	if string(h1) == string(h2) {
		...
		var s = meta.ProposalSign{
			...
			Sign:      sig,
			Threshold: commoncon.ProposalSignCount,
		}
		sByte, _ := json.Marshal(s)
		var msg = meta.P2PMessage{
			Type:    commoncon.HeaderProposalSignMsg,
...
func handleAbstractBlockHeaderSynchronizeMsg(content string) {
	...
	//}
	//将抽象区块头信息保存至本地
	redis.SetIntoRedis(commoncon.AbstractHeadersKey, content)
}

//获取本地存储的抽象区块头列表
func getLocalStoredAbHeaders() []meta.AbstractBlockHeader {
	val, err := redis.GetFromRedis(commoncon.AbstractHeadersKey)
	if err != nil {
		log.Info(err)
	}
	var abHeaders []meta.AbstractBlockHeader
...
func sendProposal(abHeaders []meta.AbstractBlockHeader, rw *bufio.ReadWriter) {
	...
	p2pMsg = meta.P2PMessage{
		Type:    commoncon.AbstractHeaderProposalMsg,
...
func BlockChainSynchronize(rw *bufio.ReadWriter) {
	for {
		...
		msg := meta.P2PMessage{
			Type:    commoncon.BlockChainSynchronizeMsg, //区块链同步
...
func BlockChainHeaderSynchronize(rw *bufio.ReadWriter) {
	for {
		...
		msg := meta.P2PMessage{
			Type:    commoncon.AbstractBlockHeaderSynchronizeMsg, //抽象区块头同步
			Content: string(bcByte),                              //区块链信息
		}
		SendP2PMessage(msg, rw)
