...
func ServerConnHandler(c net.Conn) {
	...
	for {
		...
		case common.TcpPing:
			handleTcpPing(msg, c)
		case common.TcpAbstractHeader:
			handleTcpAbstractHeader(msg, c)
		case common.TcpCrossTrans:
			//接收到对方链的跨链交易处理
...
func handleRemoteCrossTransfer(ct meta.CrossTran, c net.Conn) {
	...
	proof := ct.Proof
	//先获取到已同步的抽象区块头
	key := common.RemoteHeadersKey + ct.SourceChainId
	...
	if merkle.VerifyTranExistence(proof.TransHash, proof.MerklePath, proof.MerkleIndex, merkleRoot) {
		...
		//向对方发送自己的抽象区块头列表
		LocalAHs := chain.GetLocalAbstractBlockChainHeaders(common.LocalChainId2)
		//然后基于tcp长连接发送给对方服务节点
		...
		receMess := meta.TCPMessage{
			Type:    common.TcpCrossTransReceipt,
...
func handleTcpAbstractHeader(msg meta.TCPMessage, c net.Conn) {
	...
	//生成存储key
	key := common.RemoteHeadersKey + chainId
...
func ClientConnHandler(c net.Conn, msg meta.TCPMessage) {
	...
	case common.TcpCrossTransReceipt:
		//接收到服务端的关于交易执行的回执
...
func handleTcpCrossTransReceipt(msg meta.TCPMessage, c net.Conn) {
	...
	proof := receipt.Proof
	//先获取到已同步的抽象区块头
	key := common.RemoteHeadersKey + receipt.SourceChainId
	abHsStr, err := redis.GetFromRedis(key)
	if err != nil {
		log.Error(err)
	}
